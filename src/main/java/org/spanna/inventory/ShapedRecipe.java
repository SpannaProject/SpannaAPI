...
public class ShapedRecipe implements Recipe {
    ...
    public ShapedRecipe shape(final String... shape) {
        Validate.notNull(shape, "[ERROR] Must provide a shape");
        Validate.isTrue(shape.length > 0 && shape.length < 4, "[ERROR] Crafting recipes should be 1, 2, 3 rows, not ", shape.length);

        for (String row : shape) {
            Validate.notNull(row, "[ERROR] Shape cannot have null rows");
            Validate.isTrue(row.length() > 0 && row.length() < 4, "[ERROR] Crafting rows should be 1, 2, or 3 characters, not ", row.length());
        }
