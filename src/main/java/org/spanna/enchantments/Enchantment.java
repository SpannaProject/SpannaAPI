package org.spanna.enchantments;

import java.util.HashMap;
import java.util.Map;

import org.spanna.command.defaults.EnchantCommand;
...
public abstract class Enchantment {
    ...
    public static void registerEnchantment(Enchantment enchantment) {
        if (byId.containsKey(enchantment.id) || byName.containsKey(enchantment.getName())) {
            throw new IllegalArgumentException("[ERROR] Cannot set already-set enchantment");
        } else if (!isAcceptingRegistrations()) {
            throw new IllegalStateException("[ERROR] No longer accepting new enchantments (can only be done by the server implementation)");
        }
