...
import org.spanna.event.player.PlayerCommandPreprocessEvent;
import org.spanna.event.server.RemoteServerCommandEvent;
import org.spanna.event.server.ServerCommandEvent;
...
public class FormattedCommandAlias extends Command {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        for (String formatString : formatStrings) {
            try {
                ...
                if (throwable instanceof IllegalArgumentException) {
                    ...
                    sender.sendMessage(org.spanna.ChatColor.RED + "[ERROR] We're sorry, but an internal error occurred while attempting to perform this command");
                }
                return false;
            }
        }
        ...
        for (String command : commands) {
            result |= Spanna.dispatchCommand(sender, command);
        }
    ...
    private String buildCommand(String formatString, String[] args) {
        ...
        while (index != -1) {
            ...
            // No numbers found
            if (argStart == index) {
                throw new IllegalArgumentException("[ERROR] Invalid replacement token");
            }
            ...
            if (required && position >= args.length) {
                throw new IllegalArgumentException("[ERROR] Missing required argument " + (position + 1));
            }
