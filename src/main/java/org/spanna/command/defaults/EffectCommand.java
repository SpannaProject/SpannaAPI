...
import org.spanna.potion.PotionEffectType;
...
public class EffectCommand extends VanillaCommand {
    ...
    public EffectCommand() {
        ...
        this.setPermission("spanna.command.effect");
    }
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if (player == null) {
            sender.sendMessage(ChatColor.RED + String.format("[ERROR] Player, %s, not found", args[0]));
            return true;
        }
        ...
        if ("clear".equalsIgnoreCase(args[1])) {
            ...
            sender.sendMessage(String.format("[SERVER] Took all effects from %s", args[0]));
            return true;
        }
        ...
        if (effect == null) {
            sender.sendMessage(ChatColor.RED + String.format("[ERROR] Effect, %s, not found", args[1]));
            return true;
        }
        ...
        if (duration_temp == 0) {
            if (!player.hasPotionEffect(effect)) {
                sender.sendMessage(String.format("[ERROR] Couldn't take %s from %s as they do not have the effect", effect.getName(), args[0]));
                return true;
            }

            player.removePotionEffect(effect);
            broadcastCommandMessage(sender, String.format("[SERVER] Took %s from %s", effect.getName(), args[0]));
        } else {
            final PotionEffect applyEffect = new PotionEffect(effect, duration, amplification);

            player.addPotionEffect(applyEffect, true);
            broadcastCommandMessage(sender, String.format("[SERVER] Given %s (ID %d) * %d to %s for %d seconds", effect.getName(), effect.getId(), amplification, args[0], duration_temp));
        }
