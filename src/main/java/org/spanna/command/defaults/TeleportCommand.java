...
import org.spanna.event.player.PlayerTeleportEvent.TeleportCause;
...
public class TeleportCommand extends VanillaCommand {
    ...
    public TeleportCommand() {
        ...
        this.setPermission("spanna.command.teleport");
    }
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (args.length < 3) {
            Player target = Spanna.getPlayerExact(args[args.length - 1]);
            if (target == null) {
                sender.sendMessage("[ERROR] Can't find player " + args[args.length - 1] + ". No tp.");
                return true;
            }
            player.teleport(target, TeleportCause.COMMAND);
            Command.broadcastCommandMessage(sender, "[SERVER] Teleported " + player.getDisplayName() + " to " + target.getDisplayName());
            ...
            if (x == MIN_COORD_MINUS_ONE || y == MIN_COORD_MINUS_ONE || z == MIN_COORD_MINUS_ONE) {
                sender.sendMessage("[ERROR] Please provide a valid location!");
                return true;
            }
            ...
            Command.broadcastCommandMessage(sender, String.format("[SERVER] Teleported %s to %.2f, %.2f, %.2f", player.getDisplayName(), x, y, z));
        }
        return true;
    }
