...
public class SpreadPlayersCommand extends VanillaCommand {
    ...
    public SpreadPlayersCommand() {
        super("spreadplayers");
        this.description = "Spreads players or entities around a point";
        this.usageMessage = "/spreadplayers <x> <z> <spreadDistance> <maxRange> <respectTeams true|false> <player ...>";
        this.setPermission("spanna.command.spreadplayers");
    }
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if (distance < 0.0D) {
            sender.sendMessage(ChatColor.RED + "[ERROR] Distance is too small.");
            return false;
        }

        if (range < distance + 1.0D) {
            sender.sendMessage(ChatColor.RED + "[ERROR] Max range is too small.");
            return false;
        }
        ...
        if (respectTeams.equalsIgnoreCase("true")) {
            ...
            sender.sendMessage(String.format(ChatColor.RED + "[ERROR]'%s' is not true or false", args[4]));
            return false;
        }
        ...
        for (int i = 5; i < args.length; i++) {
            Player player = Spanna.getPlayerExact(args[i]);
            if (player == null) {
                continue;
            }
        ...
        if (rangeSpread == -1) {
            sender.sendMessage(String.format("[ERROR] Could not spread %d %s around %s,%s (too many players for space - try using spread of at most %s)", spreadSize, teams ? "teams" : "players", x, z));
            return false;
        }

        final double distanceSpread = spread(world, players, locations, teams);

        sender.sendMessage(String.format("[SERVER] Succesfully spread %d %s around %s,%s", locations.length, teams ? "teams" : "players", x, z));
        if (locations.length > 1) {
            sender.sendMessage(String.format("([SERVER] Average distance between %s is %s blocks apart after %s iterations)", teams ? "teams" : "players",  String.format("%.2f", distanceSpread), rangeSpread));
        }
        return true;
    }
