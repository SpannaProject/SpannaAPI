...
public class WeatherCommand extends VanillaCommand {
    ...
    public WeatherCommand() {
        ...
        this.setPermission("spanna.command.weather");
    }
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        World world = Spanna.getWorlds().get(0);
        ...
        if ("clear".equalsIgnoreCase(args[0])) {
            ...
            Command.broadcastCommandMessage(sender, "[SERVER] Changed weather to clear for " + (duration / 20) + " seconds.");
        } else if ("rain".equalsIgnoreCase(args[0])) {
            world.setStorm(true);
            world.setThundering(false);
            Command.broadcastCommandMessage(sender, "[SERVER] Changed weather to rainy for " + (duration / 20) + " seconds.");
        } else if ("thunder".equalsIgnoreCase(args[0])) {
            world.setStorm(true);
            world.setThundering(true);
            Command.broadcastCommandMessage(sender, "[SERVER] Changed weather to thundering " + (duration / 20) + " seconds.");
        }
