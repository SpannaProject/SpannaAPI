...
public class BanIpCommand extends VanillaCommand {
    ...
    public BanIpCommand() {
        ...
        this.setPermission("spanna.command.ban.ip");
    }
    ...
    private void processIPBan(String ip, CommandSender sender, String reason) {
        ...
        for (Player player : Spanna.getOnlinePlayers()) {
            if (player.getAddress().getAddress().getHostAddress().equals(ip)) {
                player.kickPlayer("We're sorry, but your IP has been banned from this server.");
            }
        }

        Command.broadcastCommandMessage(sender, "[SERVER] Banned IP Address " + ip);
    }
