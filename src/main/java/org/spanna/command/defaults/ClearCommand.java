...
public class ClearCommand extends VanillaCommand {
    ...
    public ClearCommand() {
        ...
        this.setPermission("spanna.command.clear");
    }
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        Player player = null;
        if (args.length > 0) {
            player = Spanna.getPlayer(args[0]);
        ...
        if (player != null) {
            ...
            if (args.length > 1 && !(args[1].equals("-1"))) {
                ...
                if (material == null) {
                    sender.sendMessage(ChatColor.RED + "[ERROR] There's no item called " + args[1]);
                    return false;
                }
            ...
            Command.broadcastCommandMessage(sender, "[SERVER] Cleared the inventory of " + player.getDisplayName() + ", removing " + count + " items");
        } else if (args.length == 0) {
            sender.sendMessage(ChatColor.RED + "[ERROR] Please provide a player!");
        } else {
            sender.sendMessage(ChatColor.RED + "[ERROR] Can't find player " + args[0]);
        }
