...
public class PlaySoundCommand extends VanillaCommand {
    public PlaySoundCommand() {
        ...
        this.setPermission("spanna.command.playsound");
    }
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        final Player player = Spanna.getPlayerExact(playerArg);
        if (player == null) {
            sender.sendMessage(ChatColor.RED + "[ERROR] Can't find player " + playerArg);
            return false;
        }
        ...
        if (location.distanceSquared(soundLocation) > fixedVolume * fixedVolume) {
            if (minimumVolume <= 0.0D) {
                sender.sendMessage(ChatColor.RED + "[ERROR] " + playerArg + " is too far away to hear the sound");
                return false;
            }
        ...
        sender.sendMessage(String.format("[SERVER] Played '%s' to %s", soundArg, playerArg));
        return true;
    }
}
