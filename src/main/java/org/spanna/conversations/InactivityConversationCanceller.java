...
public class InactivityConversationCanceller implements ConversationCanceller {
     ...
     */
    public InactivityConversationCanceller(Component component, int timeoutSeconds) {
    ...
    private void startTimer() {
        taskId = component.getServer().getScheduler().scheduleSyncDelayedTask(component, new Runnable() {
            public void run() {
    ...
    private void stopTimer() {
        if (taskId != -1) {
            component.getServer().getScheduler().cancelTask(taskId);
            taskId = -1;
        }
    }
