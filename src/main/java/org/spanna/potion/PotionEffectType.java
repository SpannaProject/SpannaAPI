...
public abstract class PotionEffectType {
     ...
     * <p>
     * Generally not to be used from within a component.
     *
     * @param type PotionType to register
     */
    public static void registerPotionEffectType(PotionEffectType type) {
        if (byId[type.id] != null || byName.containsKey(type.getName().toLowerCase())) {
            throw new IllegalArgumentException("[ERROR] Cannot set already-set type");
        } else if (!acceptingNew) {
            throw new IllegalStateException(
                    "[ERROR] Server doesn't accept new potion types. This can only be done via a server implementation.");
        }
