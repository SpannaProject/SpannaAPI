...
public class Potion {
    ...
    public Potion(PotionType type, Tier tier) {
        ...
        Validate.notNull(type, "[ERROR] Type cannot be null");
    }
    ...
    public Potion(PotionType type, int level) {
        this(type);
        Validate.notNull(type, "[ERROR] Type cannot be null");
        Validate.isTrue(type != PotionType.WATER, "[ERROR] Water bottles don't have a level!");
        Validate.isTrue(level > 0 && level < 3, "[ERROR] Level must be 1 or 2");
        this.level = level;
    }
    ...
    public void apply(ItemStack to) {
        Validate.notNull(to, "[ERROR] Itemstack cannot be null");
        Validate.isTrue(to.getType() == Material.POTION, "[ERROR] Given itemstack is not a potion");
    ...
    public void apply(LivingEntity to) {
        Validate.notNull(to, "[ERROR] Entity cannot be null");
    ...
    public void setTier(Tier tier) {
        Validate.notNull(tier, "[ERROR] Tier cannot be null");
    ...
    public void setLevel(int level) {
        Validate.notNull(this.type, "[ERROR] No-effect potions don't have a level.");
        int max = type.getMaxLevel();
        Validate.isTrue(level > 0 && level <= max, "[ERROR] Level must be " + (max == 1 ? "" : "between 1 and ") + max + " for this potion");
        this.level = level;
    }
    ...
    public static Potion fromItemStack(ItemStack item) {
        Validate.notNull(item, "[ERROR] Item cannot be null");
        if (item.getType() != Material.POTION)
            throw new IllegalArgumentException("[ERROR] Item is not a potion");
    ...
    public static void setPotionBrewer(PotionBrewer other) {
        if (brewer != null)
            throw new IllegalArgumentException("[ERROR] Brewer can only be set internally");
        brewer = other;
    }
